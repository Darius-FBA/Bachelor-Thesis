...
parser.add_argument('--name', help="compiled output dir")
parser.add_argument('--cmaddr', help="IP:port of WSE")
...
np.random.seed(42)
A = np.triu(np.random.rand(N, N).astype(np.float32)) + np.eye(N, dtype=np.float32)
b = np.random.rand(N).astype(np.float32)
x = np.zeros(N, dtype=np.float32)
x_expected = solve_triangular(A, b, lower=False)
...
runner.memcpy_h2d(A_symbol, np.tile(A.flatten(), width), 0, 0, width, 1, N * N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
runner.memcpy_h2d(b_symbol, np.tile(b, width), 0, 0, width, 1, N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
runner.memcpy_h2d(x_symbol, np.tile(x, width), 0, 0, width, 1, N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)

runner.call("f_sync", [], nonblock=False)
runner.call("f_tic", [], nonblock=False)
runner.call("f_trsv", [], nonblock=False)
...
x_result = np.zeros(N * width, dtype=np.float32)
runner.memcpy_d2h(x_result, x_symbol, 0, 0, width, 1, N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
...
for pe in range(width):
    ...
    np.testing.assert_allclose(x_result[start:end], x_expected, atol=0.01, rtol=0)
print("TRSV PARALLEL SUCCESS!")
