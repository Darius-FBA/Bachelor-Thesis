...
x_host = np.tile(x, width)
y_host = np.tile(y, width)
A_host = np.zeros(width * M * N, dtype=np.float32)

A_expected = np.outer(x, y) * alpha
A_expected_flat = A_expected.flatten()
...
runner.memcpy_h2d(x_symbol, x_host, 0, 0, width, 1, M,
    streaming=False, order=MemcpyOrder.ROW_MAJOR,
    data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
runner.memcpy_h2d(y_symbol, y_host, 0, 0, width, 1, N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR,
    data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
runner.memcpy_h2d(A_symbol, A_host, 0, 0, width, 1, M * N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR,
    data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)

runner.call("f_sync", [], nonblock=False)
runner.call("f_tic", [], nonblock=False)
runner.call("f_ger", [], nonblock=False)
...
A_result_flat = np.zeros(width * M * N, dtype=np.float32)
runner.memcpy_d2h(A_result_flat, A_symbol, 0, 0, width, 1, M * N,
    streaming=False, order=MemcpyOrder.ROW_MAJOR,
    data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
...
A_result = A_result_flat.reshape((width, M * N))
for i in range(width):
    np.testing.assert_allclose(A_result[i], A_expected_flat, rtol=1e-5, atol=1e-3)

print("GER PARALLEL SUCCESS!")
...
elapsed_cycles = max(ends) - min(starts)
print(elapsed_cycles)
